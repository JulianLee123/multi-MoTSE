...
class MoTSE(object):
    ...
    def extract_knowledge(self,task,model_path,probe_data):
        ...
        model = load_model(source_model_path=model_path).to(self.device)
    ...
    def cal_sim(self,source_tasks,target_tasks,
        ...
        for task,model_path in zip(target_tasks,target_model_paths):
            lk, gk = self.extract_knowledge(task,model_path.replace('1000','10000'),probe_data)
            t_lk.append(lk)
            t_gk.append(gk)
        ...
        for i, target_task in enumerate(target_tasks):
            ...
            makedir(result_path)
            pd.DataFrame(results_dict).to_csv(result_path+f'{target_task}.csv',
